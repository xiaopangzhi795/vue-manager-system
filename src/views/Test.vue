      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <el-card shadow="hover">
            ...
            <span >{{(data.poseData=$route.query)}}</span>
            <span >{{}}</span>
          </el-card>
        </el-col>
      </el-row>
      ...
      const onRegister = () => {
        ...
        formRefRegister.value.validate((valid) => {
          if (valid) {
            ...
            request.get("http://127.0.0.1:8080/xxl-sso-server/sso/check/" + formRegister.username).then(res => {
              console.log(res)
              if (!(res.code === 200)) {
              ...
              let key = res.data;
              console.log(key);
              let md5Pwd = Rsa.enMd5_32(formRegister.password);
              ...
              request({
                url: "http://127.0.0.1:8080/xxl-sso-server/sso/register",
                method: 'post',
                params: {
                  "data": enDataStr,
                  "key": rsaKeyStr
      ...
      const onLogin = () => {
        ...
        formRefLogin.value.validate((valid) => {
          if (valid) {
            ...
            request.get("http://127.0.0.1:8080/xxl-sso-server/sso/check/" + formLogin.username).then(res => {
              console.log(res)
              if (!(res.code === 200)) {
                ElMessage.error(JSON.stringify(res.data));
                return;
              ...
              let key = res.data;
              let md5Pwd = Rsa.enMd5_32(formLogin.password);
              console.log("MD5:" + md5Pwd);
              ...
              request({
                url: "http://127.0.0.1:8080/xxl-sso-server/sso/doLogin?redirect_url=" + data.poseData.redirect_url,
                method: 'post',
                params: {
                  "data": enDataStr,
                  "key": rsaKeyStr,
